...
`task checked -- <sha>` writes the file by hand, and takes a `YYYY-MM-DD` after the commit to override the day.
`task checked:check` reports a README and a file that have drifted apart.
